...
import concurrency.supervisor.IsolatedLifeCycleSupervisor._
import concurrency.supervisor.{IsolatedStopSupervisor, OneForOneStrategyFactory, IsolatedResumeSupervisor}
    ...
    val controls = context.actorOf(
      Props(new IsolatedResumeSupervisor with OneForOneStrategyFactory {
        override def childStarter(): Unit = {
          ...
          val ind = context.actorOf(Props[HeadingIndicator], "indicator")
          context.actorOf(Props(new ControlSurfaces(self, alt, ind)), "ControlSurfaces")
        }
      }), "Equipment"
    )
  ...
  def startPeople(): Unit = {
    ...
    val people = context.actorOf(
      Props(new IsolatedStopSupervisor with OneForOneStrategyFactory {
        override def childStarter(): Unit = {
          context.actorOf(Props(newPilot(plane, controls, altimeter, heading)), pilotName)
